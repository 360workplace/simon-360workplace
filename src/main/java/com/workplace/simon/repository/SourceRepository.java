...
public interface SourceRepository extends JpaRepository<Source, Long> {
    ...
    @Query(value = "SELECT source0_.id as id, source0_.active as active, source0_.detail as detail, source0_.fecha_inicial as fecha_inicial, source0_.titulo as titulo, source0_.type as type, source0_.fuente as fuente, source0_.responsable as responsable from linea_base source0_ INNER JOIN users u ON (source0_.fuente = u.id) INNER JOIN area a ON (u.area_id = a.id) WHERE a.id = (:areaId) AND source0_.active=true", nativeQuery = true)
